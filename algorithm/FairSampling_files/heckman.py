...
def linear_regression_heckman_correction_fairness(X_selection, X_prediction, heckman_y, X_train, y_observe, y_unobserve, y_train, discriminated_label):
    # Apply Heckman correction
    res, params_pred, params_selection, inverse_mills = Heckman(heckman_y, X_prediction, X_selection).fit(method='twostep')

    # Add inverse Mills ratio to prediction equation
    X_correct = np.hstack((X_prediction, inverse_mills))
    y_pred_train = X_correct.dot(params_pred)

    train_length = len(y_observe)

    # Only consider observed data
    y_train_pred = y_pred_train[:train_length]
    
    # Get discriminated and non-discriminated data
    discriminated_label_train = discriminated_label[:train_length]
    y_train = y_observe[:train_length].reshape(len(y_observe), 1)
    discriminated_label_train = discriminated_label_train.reshape(len(y_train), 1)
    
    y_train_label = np.hstack([y_train, discriminated_label_train])

    # Split discriminated and non-discriminated groups
    y_train_discriminated = [y_train_label[i][0] for i in range(len(y_train_label)) if y_train_label[i][1] == 1.0]
    y_train_non_discriminated = [y_train_label[i][0] for i in range(len(y_train_label)) if y_train_label[i][1] == 0.0]

    X_train = X_correct[:train_length]
    X_train_discriminated = np.array([X_train[i] for i in range(len(y_train_label)) if y_train_label[i][1] == 1.0])
    X_train_non_discriminated = np.array([X_train[i] for i in range(len(y_train_label)) if y_train_label[i][1] == 0.0])

    # Add fairness constraint
    d = np.sum(X_train_discriminated, axis=0) / len(X_train_discriminated) - np.sum(X_train_non_discriminated, axis=0) / len(X_train_non_discriminated)
    X_2TX_2 = np.matmul(X_train.T, X_train)

    nominator = np.matmul(d.T, inv(X_2TX_2)).dot(np.matmul(X_train.T, y_train_label[:, 0]))
    denominator = np.matmul(d.T, inv(X_2TX_2)).dot(d)
    lam = nominator / denominator
    beta = inv(X_2TX_2).dot(np.matmul(X_train.T, y_train_label[:, 0]) - lam * d)

    y_train_pred = X_correct.dot(beta)
    y_train_pred = y_train_pred[:train_length]

    # Reshape predictions
    y_train_pred = y_train_pred.reshape(len(y_train_pred), 1)

    # Split predicted values by race
    y_train_label_pred = np.hstack([y_train_pred, discriminated_label_train])
    y_train_discriminated_pred = [y_train_label_pred[i][0] for i in range(len(y_train_label_pred)) if y_train_label_pred[i][1] == 1.0]
    y_train_non_discriminated_pred = [y_train_label_pred[i][0] for i in range(len(y_train_label_pred)) if y_train_label_pred[i][1] == 0.0]

    return X_train, X_train_discriminated, X_train_non_discriminated, y_train, y_train_discriminated, y_train_non_discriminated, params_selection
