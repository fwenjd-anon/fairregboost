...
class DrugConsumptionBinaryDataset(BaseDataset):
    ...
    def process(self):
        ...
        self._train.loc[:,'target'] = [float(item == 'CL0') for item in _train_target]
        self._test.loc[:,'target'] = [float(item == 'CL0') for item in _test_target]

if __name__ == '__main__':
    data = DrugConsumptionBinaryDataset(data_dir='../datafiles')
